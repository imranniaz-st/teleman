...
function save_to_flow_and_copy(){
    ...
    // alert when done
    document.querySelector('p[id="clipboardStatusMessage"]').innerHTML = "JSON updated and copied to clipboard! Please import to twilio studio now.";
}
// SAVE TO FLOW AND COPY
...
function removeBranch(branchRowId) {
    var row = document.getElementById(branchRowId);
    if (row) {
        ...
        console.error('No row found with ID:', branchRowId);
    }
}

// VIS NETWORK

// JSON data for the IVR tree
    const ivrJson = updatedJson;

function buildTextualRepresentation(stateName, json, depth = 0, visited = new Set()) {
    if (visited.has(stateName)) return ''; // Avoid infinite recursion
    visited.add(stateName);

    const state = json.states.find(state => state.name === stateName);
    if (!state) return '';

    let representation = `${' '.repeat(depth * 2)}${state.name}\n`;

    state.transitions.forEach(transition => {
        let prefix = `${' '.repeat((depth + 1) * 2)}`;
        let transitionText = transition.event ? `${transition.event}` : 'Transition';
        representation += `${prefix}├── ${transitionText}\n`;
        
        if (transition.next) {
            representation += buildTextualRepresentation(transition.next, json, depth + 2, visited);
        }
    });
...
const initialStateName = "Trigger"; // Assuming "Trigger" is your starting state
const textualRepresentation = buildTextualRepresentation(initialStateName, ivrJson);

// Display the textual representation in the element with ID 'network'
document.getElementById('network').textContent = textualRepresentation;
